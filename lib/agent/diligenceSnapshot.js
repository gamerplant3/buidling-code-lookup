...
export function buildSiteFromDiligence(snap) {
  ...
  const address = snap.address || snap.label || '';
  const nameCandidate = snap.name && !/scout/i.test(snap.name) ? snap.name : address;
  const name = (nameCandidate || address || 'New site').slice(0, 80);
