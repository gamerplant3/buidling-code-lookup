/**
 * Site fields for portfolio search_portfolio tool and applyFilters.
...
/** Map common agent shorthand to nested assessment fields. */
export function normalizeFilterField(field) {
  const map = {
    pass: 'reserveAssessment.pass',
    deltaKPa: 'reserveAssessment.deltaKPa',
    'assessment.pass': 'reserveAssessment.pass',
  };
  return map[field] || field;
}

export function normalizeFilterPlan(plan) {
  if (!plan?.filters?.length) return plan;
  return {
    ...plan,
    filters: plan.filters.map((f) => ({
      ...f,
      field: normalizeFilterField(f.field),
    })),
    sort: plan.sort?.field
      ? { ...plan.sort, field: normalizeFilterField(plan.sort.field) }
      : plan.sort,
  };
}
